// CategoryPage.jsx - Premium Category Page
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-6 mb-10 border border-gray-100">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <h2 className="text-2xl font-bold text-gray-900">Filter & Sort</h2>
        <p className="text-gray-600">Showing {shown} of {total} items</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div>
          ...
          <select
            ...
            className="w-full px-5 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 transition"
          >
...
export default function CategoryPage() {
  ...
  useEffect(() => { if (catName) fetchProducts(); }, [catName]);
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE_URL}/category/${encodeURIComponent(catName)}?limit=100`);
      const data = await res.json();
      await sleep(DELAY);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-indigo-100">
      <style>{styles}</style>
      <Navbar searchTerm="" onSearchChange={handleSearch} selectedCategory={catName} onCategoryChange={handleCategory} categories={categories} loading={loading && !products.length} />

      <main className="container mx-auto px-6 py-12">
        <div className="text-center mb-12">
          <h1 className="text-5xl font-bold text-gray-900 mb-3">
            Category: <span className="gradient-text capitalize">{catName}</span>
          </h1>
          <p className="text-xl text-gray-600">Explore {total} premium products</p>
        </div>
          ...
          <div className="text-center py-24">
            <div className="bg-white rounded-3xl shadow-2xl p-16 max-w-lg mx-auto">
              <svg className="w-28 h-28 mx-auto text-gray-400 mb-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeWidth={2} d="M20 7l-2-4H6L4 7m16 0v10a2 2 0 01-2 2H6a2 2 0 01-2-2V7m16 0H4" />
              </svg>
              <p className="text-3xl font-bold text-gray-800">No products in this category</p>
            </div>
          </div>
        ) : (
          <>
            ...
            {!hasMore && <div className="text-center py-16 text-xl font-medium text-gray-700">End of category</div>}
          </>
        )}
      </main>
    </div>
